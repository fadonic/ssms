<?php
//include "dbfunctions.php";
include "header.php";
include('nav_bar.php');
?>
<?php
 
?>
<form action="view_stud_result.php" method="post" enctype="multipart/form-data" onsubmit="return validate_form(this)">
 <fieldset>
 <legend><strong>Viewing Entire Term Results</strong></legend>
      <table width="700" style="border: 1px solid #d5dfec;" align='center'>
        <!--DWLayoutTable-->
        <tr>
          <td colspan="18" align="left" valign="middle" class="ruler" bgcolor="#dddddd"><h2>Viewing Cumulative Results</h2></td>
        </tr>
        <tr>
          <td colspan="18" align="left" valign="middle" class="ruler">&nbsp;</td>
        </tr>
        <tr>
          <td  align="left" valign="middle"><strong>Current Class:</strong></td>
          <td  align="left" valign="middle"><select name="class_Id">
            <?php
                        $query = "SELECT * FROM class ORDER BY class_Id";
						$result = mysql_query($query) or die("Error ".mysql_errno().": ".mysql_error()."<br><br>$query");
						$index = 0;
						while($row = mysql_fetch_assoc($result)) {
							$index++;
					   ?>
            <option value="<?php echo $row['class_Id'] ?>"><?php echo $row['class_Name'] ?></option>
            <?php } 
                       ?>
          </select></td>
          <td >&nbsp;</td>
          <td  align="left" valign="middle"><strong>Session:</strong></td>
          <td  align="left" valign="middle"><select name="session_Id">
            <?php  
                        $query = "SELECT * FROM sessions ORDER BY session_Id";
						$result = mysql_query($query) or die("Error ".mysql_errno().": ".mysql_error()."<br><br>$query");
						$index = 0;
						while($row = mysql_fetch_assoc($result)) {
							$index++;
					   ?>
            <option value="<?php echo $row['session_Id'] ?>"><?php echo $row['session_Name'] ?></option>
            <?php } 
                       ?>
          </select></td>
        </tr>
        <tr>
          <td  align="left" valign="middle"><strong>Term:</strong></td>
          <td  align="left" valign="middle"><select name="term_Id">
            <?php
                        $query = "SELECT * FROM terms ORDER BY term_Id";
						$result = mysql_query($query) or die("Error ".mysql_errno().": ".mysql_error()."<br><br>$query");
						$index = 0;
						while($row = mysql_fetch_assoc($result)) {
							$index++;
					   ?>
            <option value="<?php echo $row['term_Id'] ?>"><?php echo $row['term_Name'] ?></option>
            <?php } 
                       ?>
          </select></td>
          <td >&nbsp;</td>
          <td  align="left" valign="middle"><!--DWLayoutEmptyCell-->&nbsp;</td>
          <td  align="left" valign="middle"><!--DWLayoutEmptyCell-->&nbsp;</td>
        </tr>
        <tr>
          <td  align="left" valign="middle">&nbsp;</td>
          <td  align="left" valign="middle"><input type="hidden" name="action" value="Browse Course Reg" /></td>
          <td align="center"><input type="submit" name="submit" value="View" /></td>
          <td  align="left" valign="middle"><strong>&nbsp;</strong></td>
          <td  align="left" valign="middle">&nbsp;</td>
        </tr>
      </table>
      </fieldset>
      </form>
      </div>
  
</div>
</body>
</html>